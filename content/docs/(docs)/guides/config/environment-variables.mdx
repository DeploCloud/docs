...
<Callout type="warn" title="A build argument stays in the image history">
...
<TypeTable
  type={{
    ...
    Shared: {
      type: "tab",
      description: "The team's shared variables, plus any another team shares with it",
    },
  }}
/>
...
- **Ticking a shared variable's scope does not inject it.** Only the per-app link does, unless the variable covers more than one team - that one adds itself everywhere. See [Shared variables](/guides/config/shared-variables).
