...
- **Stop undoes the whole run, in any step.** Everything it created here is removed with its
  data, whatever it had stopped on the old panel is started again, and the old panel keeps all
  of its own data.
- **A run that fails on its own is not rolled back.** What is already here stays, and a service
  whose data did not arrive is marked and refuses to deploy, so it cannot start on empty storage:
  copy the data again, or press **Deploy anyway** to accept it.
