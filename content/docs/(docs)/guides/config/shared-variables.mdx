...
Unticking a team turns the automatic add-in off, and the variable goes back to being a suggestion. A team that is **deleted** does not: the variable keeps injecting into the teams that are left, which is why removing a colleague's team never silently strips a value from yours.
