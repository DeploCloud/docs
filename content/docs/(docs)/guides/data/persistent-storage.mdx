...
- **Moving to another server copies the data first, and only then tears the
  old one down.** If a volume is not on the old server, a database move
  stops and stays where it is - a volume that cannot be found is also what
  a wrong volume name looks like, and the old copy is the only one there
  is. An app move says so and carries on.
</Callout>
