...
| **Deploy key** | What a deploy targets, which is the app's slug. The stack Docker sees is `deplo-<key>`, and a pull request preview appends `__pr-<n>`. |
...
| **MCP server** | The endpoint an AI agent connects to, exposing its tools, all of which run against the same schema and the same gates a person gets. |
...
<Cards>
  ...
  <Card
    ...
    title="Capabilities"
    description="The full list of 44"
