...
One script does both jobs: `uninstall.sh`. `--agent-only` is what keeps it to the agent, and it is
already in the command the dashboard prints for you. On the machine running the control plane it
is not quite a no-op for the panel: that host's Traefik goes with the agent, which is the only
thing serving the dashboard, so the panel stops answering until you re-run `install.sh`. The
script prints an SSH tunnel to get back in meanwhile. That printed command downloads the script first and runs it second (`curl -fsSL '.../uninstall.sh' --output '/tmp/deplo-uninstall.sh' && sudo bash /tmp/deplo-uninstall.sh --yes --agent-only`); the piped forms below do the same job when you type it yourself.
...
It never touches Docker Engine itself, and never touches a container Deplo did not label. It is safe on a machine that was never a Deplo host, and safe to run twice. Full flag reference, including exactly what `--purge-data` adds: [uninstall.sh](/reference/installers/uninstall). If the panel is already unreachable, `https://deplo.build/uninstall.sh` serves the same script, always at its newest version, so a dead instance never blocks the cleanup.

<Callout type="warn" title="/var/lib/deplo-agent holds your issued certificates">
Removing it takes Traefik's `acme.json`, and your certificates, with it. Let's Encrypt
rate-limits reissuance, so copy that file first if you plan to reinstall on the same host with
the same domains.
</Callout>

On the machine running the control plane that directory's `traefik` entry is a **symlink into
`/opt/deplo`**, so removing it takes the link rather than the certificates behind it. There the
certificates live with the rest of `/opt/deplo`.
