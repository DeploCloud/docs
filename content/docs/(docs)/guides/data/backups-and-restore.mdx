...
Pick any server in the fleet, including a [storage-only](/advanced/server-roles) box that runs nothing else. Leave the path empty to use the agent's own managed store: naming a path instead requires an absolute one and instance admin.
...
Set the schedule and its **Timezone**. The frequencies start at **every 15 minutes**: a dump any
more often than that pins the host's disk and holds up every other backup on the instance.
...
**Keep is a count, not a number of days.** "Keep 7" - the default - means the seven most recent
successful artifacts, whether they were taken over a week or a year. Older ones are removed after
each successful run, the newest successful artifact is never removed, and a failed run consumes no
slot. The ceiling is 365, and a target holds at most 50 recorded runs.
...
**Download** in a restore point's menu. Every successful artifact is downloadable wherever it is
kept, bucket or server disk, and it is decrypted on the way out. Downloading needs
`restore_backups`; an **app** archive additionally needs `reveal_secrets`, because it carries that
app's decrypted environment.
