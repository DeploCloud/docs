...
One variable that several apps can use, across one team or across several. Change it once and
every app that uses it gets the new value on its next deploy: an API key three services all call,
an S3 bucket name, a `SENTRY_DSN`.

A shared variable has two independent parts, and confusing them is the single most common mistake
here.
...
You can only tick a team you hold **Manage environment variables** in, across the whole of that team. A team the variable already reaches stays ticked and locked, so a variable shared instance-wide is still editable by an admin who is not a member of every team.
...
<Callout type="info">
Creating a shared variable needs the **Manage environment variables** permission held across the whole team, not only on one app. Without it the button is not there.
</Callout>
...
A linked row in an app offers two actions. **Manage** opens the variable on **Variables ->
Shared**, where its audience and its value live. **Remove from this app** unlinks it here, and the
variable and every other app keep it. Deleting the variable outright is done from that Shared tab,
not from an app.

On a variable that reaches more than one team there is no link to take back: **Remove from this
app** drops your explicit one, and the variable keeps arriving from the team that shares it, at
the lowest priority. The row says so, and stays.
...
- **A preview built from a fork receives no shared value at all**, plain or secret, along with
  none of the app's own. It gets its preview overrides and nothing else.
