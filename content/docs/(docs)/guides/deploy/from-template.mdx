...
Open **Templates** in the sidebar, or pick **Template** on the first step of the
new-app wizard.
...
- **Deploying the same template twice into one environment renames what would clash.** Two stacks cannot both answer to `db` on one network, so the second one gets `myapp-db`, references included, and the app's Activity says so. See [Network isolation](/advanced/network-isolation#two-stacks-one-name).
