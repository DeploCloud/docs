...
<Callout type="warn" title="The dashboard is never reachable at <server-ip>:3000">
  Port 3000 is published on `127.0.0.1` only. A panel on an open port is a login page on the internet with no TLS in front of it. If the proxy itself is what broke, the way in is an SSH tunnel: `ssh -L 3000:localhost:3000 root@your-server`, then open `http://localhost:3000`.
</Callout>
...
- `ghcr.io/deplocloud/deplo`, publishing port `3000` **on `127.0.0.1` only**.
